...
function Home() {
	const { BetList, BetResults, fetchAllResults, setBetResults } = useBet();
	const { User } = useUser();
	...
	// handlers--------------
	function handleTabClick(value: "My Created bets" | "Results") {
		if (value == "My Created bets") {
			setStep(1);
			setTabMode(value);
		} else {
			setStep(2);
...
// console.log(currentlist[0]);
	...
	// set users list 
	function FilterUserBet()
	{
		const list = BetList.filter((i :any) => i.Creator._id == User._id);
			...
			<main className="dashboard_home bg-white w-full h-auto">
				...
				{step == 2 && (
					<div className="w-full h-auto px-4 md:px-12 pt-1/2 ">
						...
						{BetResults.length > 0 ? (
							<div className="active_bet_wrapper pb-36 grid md:grid-cols-2 lg:grid-cols-3 gap-6 w-full  h-auto mt-6 ">
								{BetResults.map((i: any, k: number) => (
									<div className="" key={k}>
										<ResultCard betType={i.Type} result={i} />
									</div>
								))}
							</div>
						) : (
