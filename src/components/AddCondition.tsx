...
const AddCondition = ({ toggle, showNoti }: PropTypes) => {
  ...
  function handleSetCondition(condition: ConditionTypes) {
    ...
    if (updatedArray.length > 0) {
      let existing = updatedArray.filter(
        (i: ConditionTypes, a) => i.id === condition.id
      );
  ...
  function handleRemoveItem(condition: any) {
    ...
    // find existing condition
    let existing = currentArray.filter(
      (i: ConditionTypes, a) => i.id === condition.id
    );
          ...
          <div className="  w-full p-8">
            ...
            <div className="w-full column text-gray-500">
              ...
              <p className="txt-sm mt-2 w-[430px]">
                Add conditions to make your bet more exciting. Choose from a
                range of options to create a unique challenge
              </p>
            </div>
            ...
            <div className="selector-wrapper w-full mt-4 ">
                  ...
                  >
                    ...
                    <ol className="team_options w-full p-4">
                      ...
                      {sectors.map((i, k) => (
                        ...
                        >
                          <div
                            className={`h-full w-1 rounded-lg ${i.color}`}
                          ></div>

                          <h1 className="text-gray-500 txt-sm f-m capitalize">
                            {i.type}
                          </h1>
                        </li>
                      ))}
              ...
              <div className="selector mt-4">
                <label className="txt-sm f-b text-gray-600">
                  {" "}
                  Select conditions
                </label>
                ...
                <div
                  role="button"
                  className="selector_btn row-between p-2 border rounded-lg mt-[10px]"
                >
                  {!conditions.length ? (
                    <p
                      onClick={handleShowConditionList}
                      className="txt-sm p-2 f-m text-gray-500  w-full h-full"
                    >
                      Select conditions
                    </p>
                  ) : (
                      ...
                      {conditions.map((i, k) => (
                        ...
                        >
                          <h1 className="f-m text-gray-700">
                            {i.betType}
                            {":"}
                            <span className="text-gray-400 txt-sm f-m ml-1">
                              {i.name}
                            </span>
                          </h1>
                ...
                {showConditionList && (
                  ...
                  >
                    ...
                    <ol className="team_options w-full px-4 p-2 space-y-2">
                      <p className="txt-xs f-b text-gray-900">
                      ...
                      {conditionList.map((i, k) => (
                        ...
                        >
                          <div className="icon">
                            {!isExisting(i) ? <AddIcon /> : <CheckIcon />}
                          </div>
