								...
								<div className="badge_container row-between ">
									...
									<div className="col">
										...
										<div className="w-auto space-x-2 flex items-center">
											...
											<div role='button' onClick={handleCopy} className="border centered p-[14px] rounded-xl">
									...
									{tabs.map((i, k) => (
										<div
											...
											className={`tab_item px-3  hover:text-gray-700  border-b-2 space-x-2 ${
												betTabMode == i.tabMode ? "text-gray-700 border-gray-700  " : "border-transparent text-gry-500"
											} h-full middle`}
											key={k}
										>
											<p className={`txt-sm  ${betTabMode == i.tabMode ? "f-b" : "f-m"}`}> {i.name}</p>{" "}
											{i.badge && <p className="rounded bg-gray-600 px-2 p-[2px] text-white txt-xs f-m">{i.badge}</p>}
										</div>
									))}
								</div>
							</div>
...
export function PlayerAvatar({ id }: any) {
